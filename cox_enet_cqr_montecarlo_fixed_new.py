"""
Cox Elastic-Net + Conformalized Quantile Regression (CQR) — FIXED pipeline.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ROOT CAUSE OF WIDE INTERVALS (OLD CODE) AND THE FIX
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

OLD approach (month_interval_conformal):
  - IsotonicRegression fit on log1p(OS_MONTHS) vs risk_score
  - qhat computed on |log1p residuals| → qhat ≈ 1.0–2.0 in LOG space
  - Back-transformed: hi = expm1(yhat + qhat)
  - Because expm1 is convex, a log-space qhat of 1.5 maps asymmetrically:
      yhat=3.5 (≈29 months) → hi = expm1(5.0) ≈ 147 months
      yhat=2.0 (≈6 months)  → hi = expm1(3.5) ≈ 32 months
  - Result: catastrophically wide upper bounds, sometimes DECADES.

FIX — CQR (Romano, Patterson & Candès, NeurIPS 2019):
  - Two GradientBoostingRegressor quantile models at alpha/2 and 1-alpha/2
    are fit DIRECTLY on raw OS_MONTHS (no log transform).
  - Nonconformity score on calibration events:
      E_i = max(q_lo(X_i) − Y_i,  Y_i − q_hi(X_i))   [Romano eq. 9]
  - qhat is now in RAW MONTHS (e.g. 8 months), not log-months.
  - Final interval: [q_lo(X)−qhat, q_hi(X)+qhat], clipped to [0,∞).
  - Both bounds shift by the SAME absolute number of months → no explosion.
  - Width is controlled by the actual variance of event times in the data,
    not amplified by exponential back-transformation.

Additional tightening levers (see TIGHTENING_LEVERS below):
  1. Increase CQR_MIN_TRAIN_EVENTS / CQR_MIN_CAL_EVENTS thresholds
     (more data → tighter quantile estimates).
  2. Tune CQR_GBR_PARAMS: more estimators, lower learning_rate, or
     constrain max_depth for less-wiggly quantile boundaries.
  3. Post-hoc cap: MAX_INTERVAL_WIDTH_MONTHS clips any remaining outlier
     upper bounds. Set to None to disable.
  4. The interval width is fundamentally limited by the variance of true
     event times in your cohort — no algorithm can reduce it below that.
...
# Miscoverage α → target coverage = 1 − α = 90%
CONFORMAL_ALPHA = 0.10

HORIZONS_MONTHS = [12, 24, 36, 60]

# Monte Carlo survival horizon reporting (upper-plausible survival times)
MC_TARGET_MONTHS = [80, 90]
MC_N_SIMS = 5000
...
# ── CQR settings ─────────────────────────────────────────────────────────────
# Minimum event-only rows needed to fit reliable quantile models.
CQR_MIN_TRAIN_EVENTS = 20
CQR_MIN_CAL_EVENTS   = 10

# GradientBoostingRegressor params for quantile regression.
# Shared between lo and hi models; each sets its own `alpha`.
# TIGHTENING LEVER: increase n_estimators or decrease max_depth for smoother,
# less-variable quantile curves → narrower raw QR band before conformal step.
CQR_GBR_PARAMS: Dict[str, Any] = {
    "loss":              "quantile",   # pinball / check loss at level `alpha`
    "n_estimators":      300,          # more trees → more stable quantiles
    "max_depth":         3,
    "learning_rate":     0.04,
    "min_samples_leaf":  10,
...
# TIGHTENING LEVER: hard cap on interval width in months (None = no cap).
# Set to e.g. 60 to enforce that no interval exceeds 5 years regardless of
# what the conformal correction produces. This breaks the formal coverage
# guarantee for patients whose true event time falls outside [lo, lo+cap],
# so use it only when interpretability matters more than strict validity.
MAX_INTERVAL_WIDTH_MONTHS: Optional[float] = None  # e.g. 60.0

# ── Paths ────────────────────────────────────────────────────────────────────
BASE       = Path("/home/illionar/Projects/ml_research")
INPUT_PATH = BASE / "data" / "preprocessed_cleaned" / "patient_multiomic_cleaned.parquet"
OUT_DIR    = BASE / "data" / "model_outputs" / "cox_enet_cqr_fixed_new"


# ─────────────────────────────────────────────────────────────────────────────
# Cox Elastic-Net model  (unchanged from previous versions)
# ─────────────────────────────────────────────────────────────────────────────
...
class CoxElasticNet:
    ...
    Loss: −partial_log_likelihood + α [l1_ratio * smooth_L1(β) + (1−l1_ratio) * ½‖β‖²]

    Sorting by descending time ensures that the cumulative sum s0 = Σ exp(η)
    over rows 0..i equals the risk-set sum at event time t_stop[i] (Breslow
    approximation, standard for tied times).
    """
    ...
    def __init__(
        ...
        self.smooth_l1_eps = smooth_l1_eps
    ...
    def _nll_grad(
        ...
        if len(idx) == 0:
            ...
            grad     = self.alpha * (self.l1_ratio * grad_l1 + (1 - self.l1_ratio) * beta)
        ...
        grad_l1 = beta / np.sqrt(beta * beta + self.smooth_l1_eps)
        grad    = -grad_loglik + self.alpha * (self.l1_ratio * grad_l1 + (1 - self.l1_ratio) * beta)
        return float(nll), grad
...
def concordance_index_censored(
    ...
    A pair (i, j) is comparable if t_i < t_j and patient i experienced the
    event (e_i = 1). The model is concordant when s_i > s_j for such a pair.
    Tied risk scores contribute 0.5.

    Source: Harrell FE et al., JAMA 1982; 247(18):2543-2546.
    """
    ...
    s_i = risk_scores[:, None];  s_j = risk_scores[None, :]
...
# ─────────────────────────────────────────────────────────────────────────────
# Conservative conformal quantile  (Lei et al. 2018)
# ─────────────────────────────────────────────────────────────────────────────

def conformal_quantile(scores: np.ndarray, alpha: float) -> float:
    """
    Return the conservative finite-sample (1−α) empirical quantile.

    level = ceil((n+1)*(1−α)) / n
    Guarantees P(Y_new in interval) >= 1−α over exchangeable draws.
...
def prepare_dataframe() -> pd.DataFrame:
    ...
    if n_rows < 20:
        raise ValueError(f"Too few rows after cleaning: {n_rows}. Need ≥ 20.")
    if n_events == 0 or n_censored == 0:
        raise ValueError("Only one outcome class present; need both events and censored rows.")

    print(f"Data loaded: {n_rows} rows | {n_events} events | {n_censored} censored "
          f"| event_rate={n_events/n_rows:.3f}")
    return df
...
def split_three_way(
    ...
    for name, sidx in (("train", idx_train), ("calibration", idx_cal), ("test", idx_test)):
        nr = int(len(sidx));  ne = int(y[sidx].sum())
        report[name] = {"rows": nr, "events": ne, "censored": nr - ne,
...
def _drop_by_incremental_correlation(
    ...
    bad = ~np.isfinite(X)
    if np.any(bad):
        r, c = np.where(bad);  X[r, c] = medians[c]
    X    = X - np.mean(X, axis=0, keepdims=True)
    norms = np.linalg.norm(X, axis=0)
    kept_idx: List[int] = [];  drop_rows: List[Dict[str, Any]] = []
    for j, col_name in enumerate(cols):
        if not kept_idx:
            kept_idx.append(j);  continue
        if norms[j] < 1e-12:
            kept_idx.append(j);  continue
...
def drop_collinear_features(
    ...
    for col in columns:
        ...
        if int(s.notna().sum()) < 2:
            skipped_sparse.append(col);  continue
        std = float(s.std(skipna=True))
        if (not np.isfinite(std)) or std < 1e-12:
            skipped_constant.append(col);  continue
...
def apply_collinearity_filter(
    ...
    clin_numeric: List[str] = [];  clin_non_numeric: List[str] = []
    for col in clinical_cols:
        (clin_numeric if int(pd.to_numeric(df_train[col], errors="coerce").notna().sum()) >= 2
         else clin_non_numeric).append(col)
    ...
    ) if all_tables else _empty_drop_table())
    summary = {
        ...
        "feature_counts": {
            "expr_before": int(len(expr_cols)),  "expr_after": int(len(expr_kept)),
            "clinical_before": int(len(clinical_cols)), "clinical_after": int(len(clinical_final)),
...
def _fit_clinical_block(
    df_tr: pd.DataFrame, df_va: pd.DataFrame, clinical_cols: List[str],
    ...
    c_tr = df_tr[clinical_cols].copy();  c_va = df_va[clinical_cols].copy()
    ...
    if cat_cols:
        transformers.append(("cat", Pipeline([
            ("imp", SimpleImputer(strategy="most_frequent")), ("oh", _make_ohe())
        ]), cat_cols))
    if not transformers:
        return empty(len(df_tr)), empty(len(df_va)), None, [], []
    pre = ColumnTransformer(transformers=transformers, remainder="drop")
    return (
        np.asarray(pre.fit_transform(c_tr), dtype=float),
        np.asarray(pre.transform(c_va),     dtype=float),
        pre, num_cols, cat_cols,
    )


def _fit_expression_block(
    df_tr: pd.DataFrame, df_va: pd.DataFrame, expr_cols: List[str],
) -> Tuple[np.ndarray, np.ndarray, Optional[Pipeline], int]:
    empty = lambda n: np.empty((n, 0), dtype=float)
    if not expr_cols:
        return empty(len(df_tr)), empty(len(df_va)), None, 0
    e_tr = df_tr[expr_cols].copy();  e_va = df_va[expr_cols].copy()
    ...
    return (
        np.asarray(pipe.fit_transform(e_tr), dtype=float),
        np.asarray(pipe.transform(e_va),     dtype=float),
        pipe, int(n_comp),
    )


def fit_transform_features(
    df_tr: pd.DataFrame, df_va: pd.DataFrame,
    clinical_cols: List[str], expr_cols: List[str],
) -> Tuple[
    ...
    X_tr = np.hstack(blocks_tr);  X_va = np.hstack(blocks_va)
...
def run_cv_tuning(
    ...
    for cox_alpha in ALPHA_GRID:
        for l1_ratio in L1_GRID:
            fold_scores: List[float] = [];  failures: List[str] = []
            for fold_id, (tr_idx, va_idx) in enumerate(
                skf.split(np.zeros(len(df_train)), y), start=1
            ):
                tr = df_train.iloc[tr_idx];  va = df_train.iloc[va_idx]
                try:
                    ...
                    mdl.fit(X_tr, tr["OS_MONTHS"].to_numpy(float),
                            tr["OS_EVENT"].to_numpy(int))
                    if not bool(getattr(mdl, "success_", True)):
                        failures.append(f"fold_{fold_id}:not_converged");  continue
                    ...
                    if not np.isfinite(ci):
                        failures.append(f"fold_{fold_id}:non_finite_ci");  continue
...
# ─────────────────────────────────────────────────────────────────────────────
# CQR month-interval prediction  (THE FIX)
# ─────────────────────────────────────────────────────────────────────────────
...
def cqr_month_interval(
    ...
    """
    CQR survival-time intervals operating on the RAW MONTH SCALE.

    Why this fixes wide intervals
    -----------------------------
    The old code computed qhat in log1p-space (~1–2 log-months) then
    back-transformed with expm1. Because expm1 is convex, the upper bound
    explodes: expm1(yhat + 1.5) can be 30–100× larger than expm1(yhat − 1.5).

    CQR works entirely in raw months:
      - qhat is in months (e.g. 8 months), not log-months.
      - lo = q_lo(X) − qhat,   hi = q_hi(X) + qhat
      - Both bounds shift by the SAME absolute value → no asymmetric explosion.
      - Width is determined by the true variance of event times, not by
        exponentiation of log-space residuals.

    Algorithm (Romano et al. 2019, Algorithm 1)
    -------------------------------------------
    1. Fit qr_lo (quantile=α/2) and qr_hi (quantile=1−α/2) on event-only
       training rows (OS_MONTHS = true event time, fully observed).
    2. Nonconformity score on event-only calibration rows:
           E_i = max(q_lo(X_i) − Y_i,  Y_i − q_hi(X_i))
    3. Conservative conformal quantile:
           qhat = quantile(E, ceil((n+1)*(1−α)) / n)
    4. Test intervals:
           lo = max(0, q_lo(X_te) − qhat)
           hi = max(lo, q_hi(X_te) + qhat)

    Coverage guarantee: ~90% marginal coverage for exchangeable event-only
    draws. No coverage claim for censored patients.
    """
    n_te = len(X_te)
    ...
    if n_tr_ev < CQR_MIN_TRAIN_EVENTS:
        details["status"] = (
            f"skipped: only {n_tr_ev} training events (need >= {CQR_MIN_TRAIN_EVENTS})"
        )
        return lo, hi, np.nan, np.nan, details
    if n_cal_ev < CQR_MIN_CAL_EVENTS:
        details["status"] = (
            f"skipped: only {n_cal_ev} calibration events (need >= {CQR_MIN_CAL_EVENTS})"
        )
        return lo, hi, np.nan, np.nan, details

    X_tr_ev  = X_tr[tr_mask];   y_tr_ev  = t_tr[tr_mask]   # raw months
    X_cal_ev = X_cal[cal_mask];  y_cal_ev = t_cal[cal_mask]

    # Step 1: Fit quantile models on raw months (no log transform)
    alpha_lo = alpha / 2.0;   alpha_hi = 1.0 - alpha / 2.0
    try:
    ...
    # Step 2: CQR nonconformity scores on calibration events
    q_lo_cal = qr_lo.predict(X_cal_ev)
    q_hi_cal = qr_hi.predict(X_cal_ev)
    # Quantile crossing safeguard
    q_lo_cal_raw = q_lo_cal.copy()
    q_hi_cal_raw = q_hi_cal.copy()
    ...
    details["raw_qr_interval_width_mean_cal_months"] = float(
        np.mean(q_hi_cal - q_lo_cal)
    )

    # Step 3: Conservative conformal quantile (in raw months)
    qhat = conformal_quantile(scores, alpha)
    details["qhat_months"] = float(qhat)

    if not np.isfinite(qhat):
        details["status"] = (
            f"skipped: qhat=+inf, calibration set too small "
            f"(n_cal_events={n_cal_ev}, alpha={alpha})"
        )
        return lo, hi, np.nan, np.nan, details

    # Step 4: Test-set intervals (all rows, including censored)
    q_lo_te = qr_lo.predict(X_te)
    q_hi_te = qr_hi.predict(X_te)
    q_lo_te_raw = q_lo_te.copy()
    q_hi_te_raw = q_hi_te.copy()
    q_lo_te = np.minimum(q_lo_te_raw, q_hi_te_raw)
    q_hi_te = np.maximum(q_lo_te_raw, q_hi_te_raw)

    lo = np.clip(q_lo_te - qhat, 0.0, None)   # raw months, no log
    hi = q_hi_te + qhat
    hi = np.maximum(lo, hi)                    # ensure hi >= lo

    # Optional hard cap on interval width (TIGHTENING LEVER)
    ...
    details["median_interval_width_months_all_test"] = float(np.median(all_widths))
    details["p90_interval_width_months_all_test"]    = float(np.percentile(all_widths, 90))

    # Step 5: Empirical coverage on test event rows
    if n_te_ev > 0:
        lo_ev = lo[te_mask];  hi_ev = hi[te_mask];  y_te_ev = t_te[te_mask]
...
def _horizon_labels(
    time_arr: np.ndarray, event_arr: np.ndarray, horizon: float,
...
def time_dependent_conformal(
    r_cal: np.ndarray, t_cal: np.ndarray, e_cal: np.ndarray,
    r_te:  np.ndarray, t_te:  np.ndarray, e_te:  np.ndarray,
    ...
    for h in HORIZONS_MONTHS:
        ...
        n_cal = int(len(y_cal));  n_te = int(len(y_te))
        base: Dict[str, Any] = {"horizon_months": h, "n_cal_known": n_cal, "n_test_known": n_te}

        if n_cal == 0 or n_te == 0:
            rows_metrics.append({**base, "note": "no_known_labels"});  continue
        if len(np.unique(y_cal)) < 2:
            rows_metrics.append({**base, "note": "insufficient_cal_class_variation"});  continue
        if len(np.unique(y_te)) < 2:
            rows_metrics.append({**base, "note": "insufficient_test_class_variation"});  continue
        ...
        try:
            ...
            rows_metrics.append({**base, "note": f"isotonic_failed:{type(exc).__name__}"});  continue

        if not (np.isfinite(p_cal).all() and np.isfinite(p_te).all()):
            rows_metrics.append({**base, "note": "non_finite_isotonic_probs"});  continue

        scores = np.abs(y_cal - p_cal)
        qhat_h = conformal_quantile(scores, CONFORMAL_ALPHA)
        if not np.isfinite(qhat_h):
            rows_metrics.append({**base, "note": "non_finite_conformal_quantile"});  continue

        p_lo = np.clip(p_te - qhat_h, 0.0, 1.0)
        p_hi = np.clip(p_te + qhat_h, 0.0, 1.0)
        coverage = float(np.mean((y_te >= p_lo) & (y_te <= p_hi)))
        rows_metrics.append({
            **base, "qhat": float(qhat_h), "coverage_known_test": coverage,
            "mean_interval_width": float(np.mean(p_hi - p_lo)), "note": "ok",
        })

        # Youden J threshold from calibration only
        best_thr = 0.5;  best_j = -1.0
        ...
        calibrators[int(h)] = {
            "isotonic": iso, "qhat": float(qhat_h), "threshold": float(best_thr)
        }
    ...
    return (
        pd.DataFrame(rows_metrics), pd.DataFrame(rows_pred),
        pd.DataFrame(rows_cls), calibrators,
    )


# ─────────────────────────────────────────────────────────────────────────────
# Monte Carlo survival horizon block
# ─────────────────────────────────────────────────────────────────────────────
...
def fit_breslow_baseline_hazard(
    ...
    """
    Fit a Breslow baseline cumulative hazard from Cox risk scores.

    This is used only for Monte Carlo simulation of survival times.
    """
    times = np.asarray(times, dtype=float)
    events = np.asarray(events, dtype=int)
    risk_scores = np.asarray(risk_scores, dtype=float)
    ...
    if len(times) == 0:
        return np.array([], dtype=float), np.array([], dtype=float), {
            "status": "skipped:empty_training_data"
        }
...
def simulate_cox_survival_times(
    ...
    """
    Simulate survival times from a Cox model using inverse-transform sampling.

    Output is truncated at the observed maximum follow-up in the training data.
    """
    risk_scores = np.asarray(risk_scores, dtype=float)
...
def monte_carlo_survival_block(
    ...
    n_sims: int = MC_N_SIMS,
    target_months: List[int] = MC_TARGET_MONTHS,
) -> Tuple[pd.DataFrame, Dict[str, Any], Dict[str, Any]]:
    """
    Monte Carlo survival summary for the test split.

    The output focuses on upper-plausible survival horizons and probabilities
    at clinically interpretable time points such as 80 and 90 months.
    """
    baseline_times, baseline_cumhaz, baseline_meta = fit_breslow_baseline_hazard(
        t_tr, e_tr, r_tr
    )
    ...
    if sim_times.size == 0:
        empty = pd.DataFrame(columns=[
            "test_row_index_within_split", "PATIENT_ID", "mc_survival_mean_months",
            "mc_survival_median_months", "mc_survival_q80_months",
            "mc_survival_q90_months", "mc_prob_survive_80_months",
            "mc_prob_survive_90_months", "mc_note",
        ])
        return empty, baseline_meta, {"status": "skipped:empty_test_set"}

    q50 = np.quantile(sim_times, 0.50, axis=1)
    q80 = np.quantile(sim_times, 0.80, axis=1)
    q90 = np.quantile(sim_times, 0.90, axis=1)
    mean = np.mean(sim_times, axis=1)

    prob_80 = np.mean(sim_times >= 80.0, axis=1)
    prob_90 = np.mean(sim_times >= 90.0, axis=1)
    ...
    for i in range(sim_times.shape[0]):
        rows.append({
            ...
            "mc_survival_mean_months": float(mean[i]),
            "mc_survival_median_months": float(q50[i]),
            "mc_survival_q80_months": float(q80[i]),
            "mc_survival_q90_months": float(q90[i]),
            "mc_prob_survive_80_months": float(prob_80[i]),
            "mc_prob_survive_90_months": float(prob_90[i]),
            "mc_note": "truncated_at_training_followup",
        })
    ...
    summary = {
        ...
        "simulation_note": "Monte Carlo is conditional on the fitted Cox model and the observed training follow-up.",
        "interpretation": "q80/q90 are upper-plausible survival horizons, not guarantees.",
    }
...
def main() -> None:
    ...
    # ── 3. Collinearity filter (train only) ───────────────────────────────
    ...
    print(f"After collinearity: clinical {len(clinical_cols)} → {len(clinical_cols_f)} | "
          f"expr {len(expr_cols)} → {len(expr_cols_f)}")
    ...
    print(f"Best params: alpha={best_cox_alpha}, l1_ratio={best_l1}, "
          f"CV c-index={float(best['mean_c_index']):.4f}")
    ...
    if cqr_details.get("mean_interval_width_months_all_test") is not None:
        print(f"  Mean interval width (all test): "
              f"{cqr_details['mean_interval_width_months_all_test']:.1f} months")
    if np.isfinite(coverage_ev):
        print(f"  Empirical coverage (event rows): {coverage_ev:.3f}")

    # ── 9. Monte Carlo survival horizon block ────────────────────────────
    print("Running Monte Carlo survival simulation (upper-plausible horizons) …")
    mc_te, mc_baseline_meta, mc_summary = monte_carlo_survival_block(
        t_tr, e_tr, r_tr,
        t_te, e_te, r_te,
        patient_ids_te=te["PATIENT_ID"].astype(str).to_numpy(),
        n_sims=MC_N_SIMS,
        target_months=MC_TARGET_MONTHS,
    )
    mc_te.to_csv(OUT_DIR / "monte_carlo_survival_test_predictions.csv", index=False)
    print(
        "  MC q90 survival horizon sample: "
        f"{mc_te['mc_survival_q90_months'].median():.1f} months (median across test patients)"
        if not mc_te.empty else "  MC block skipped: no test predictions"
    )

    # ── 10. Time-dependent conformal ──────────────────────────────────────
    ...
    # Monte Carlo predictions are stored only for the test split; train/calibration
    # rows are left as NaN so the file remains easy to read and audit.
    mc_fill = {
        "mc_survival_mean_months": np.full(len(tr) + len(cal), np.nan),
        "mc_survival_median_months": np.full(len(tr) + len(cal), np.nan),
        "mc_survival_q80_months": np.full(len(tr) + len(cal), np.nan),
        "mc_survival_q90_months": np.full(len(tr) + len(cal), np.nan),
        "mc_prob_survive_80_months": np.full(len(tr) + len(cal), np.nan),
        "mc_prob_survive_90_months": np.full(len(tr) + len(cal), np.nan),
    }
    mc_test_cols = {
        "mc_survival_mean_months": mc_te["mc_survival_mean_months"].to_numpy() if not mc_te.empty else np.array([]),
        "mc_survival_median_months": mc_te["mc_survival_median_months"].to_numpy() if not mc_te.empty else np.array([]),
        "mc_survival_q80_months": mc_te["mc_survival_q80_months"].to_numpy() if not mc_te.empty else np.array([]),
        "mc_survival_q90_months": mc_te["mc_survival_q90_months"].to_numpy() if not mc_te.empty else np.array([]),
        "mc_prob_survive_80_months": mc_te["mc_prob_survive_80_months"].to_numpy() if not mc_te.empty else np.array([]),
        "mc_prob_survive_90_months": mc_te["mc_prob_survive_90_months"].to_numpy() if not mc_te.empty else np.array([]),
    }
    pred_df = pd.DataFrame({
        ...
        # Conformal interval on raw months; NaN for train and calibration splits.
        "pred_survival_lo_months_90": np.concatenate([
            np.full(len(tr), np.nan), np.full(len(cal), np.nan), lo,
        ]),
        "pred_survival_hi_months_90": np.concatenate([
            np.full(len(tr), np.nan), np.full(len(cal), np.nan), hi,
        ]),
        # Monte Carlo survival summaries; NaN for train and calibration splits.
        "mc_survival_mean_months": np.concatenate([
            mc_fill["mc_survival_mean_months"],
            mc_test_cols["mc_survival_mean_months"],
        ]),
        "mc_survival_median_months": np.concatenate([
            mc_fill["mc_survival_median_months"],
            mc_test_cols["mc_survival_median_months"],
        ]),
        "mc_survival_q80_months": np.concatenate([
            mc_fill["mc_survival_q80_months"],
            mc_test_cols["mc_survival_q80_months"],
        ]),
        "mc_survival_q90_months": np.concatenate([
            mc_fill["mc_survival_q90_months"],
            mc_test_cols["mc_survival_q90_months"],
        ]),
        "mc_prob_survive_80_months": np.concatenate([
            mc_fill["mc_prob_survive_80_months"],
            mc_test_cols["mc_prob_survive_80_months"],
        ]),
        "mc_prob_survive_90_months": np.concatenate([
            mc_fill["mc_prob_survive_90_months"],
            mc_test_cols["mc_prob_survive_90_months"],
        ]),
    })
    ...
    metrics: Dict[str, Any] = {
        ...
        "fix_description":       "CQR on raw month scale replaces old isotonic+log1p approach",
        ...
        "cqr_month_interval": {
            **cqr_details,
            "mean_interval_width_months_all_test":   mean_w_all,
            "median_interval_width_months_all_test": median_w_all,
            "p90_interval_width_months_all_test":    p90_w_all,
        },
        ...
        "monte_carlo_survival": {
            **mc_summary,
            "baseline_hazard_status": mc_baseline_meta.get("status", "unknown"),
            "baseline_hazard_n_unique_event_times": int(mc_baseline_meta.get("n_unique_event_times", 0)),
            "baseline_hazard_max_followup_months": float(mc_baseline_meta.get("max_observed_followup_months", float("nan"))),
        },
    }
    ...
    artifact: Dict[str, Any] = {
        ...
        # CQR: qhat only (QR models are re-fit at inference time if needed)
        "cqr_qhat":    qhat,
        "cqr_alpha":   CONFORMAL_ALPHA,
        "cqr_details": cqr_details,
        "time_dependent_calibrators": td_calibrators,
        "horizons_months":            HORIZONS_MONTHS,
        "baseline_hazard_times":      mc_baseline_meta.get("baseline_times", None),
        "baseline_cumulative_hazard":  mc_baseline_meta.get("baseline_cumhaz", None),
        "baseline_hazard_meta":        mc_baseline_meta,
        "mc_n_sims":                   MC_N_SIMS,
        "mc_target_months":            MC_TARGET_MONTHS,
    }
    ...
    readme = [
        ...
        (f"qhat              : {qhat:.2f} months" if ok and np.isfinite(qhat)
         else "qhat              : N/A"),
        (f"Coverage (events) : {coverage_ev:.4f}" if ok and np.isfinite(coverage_ev)
         else "Coverage          : N/A"),
        (f"Mean width (all)  : {mean_w_all:.1f} months" if np.isfinite(mean_w_all)
         else "Mean width        : N/A"),
        f"MC q90 horizon    : {float(mc_te['mc_survival_q90_months'].median()):.1f} months (median across test patients)" if not mc_te.empty else "MC q90 horizon    : N/A",
        "",
        ...
        "3. Keep Monte Carlo outputs as upper-plausible horizons only.",
        "4. Set MAX_INTERVAL_WIDTH_MONTHS = e.g. 36 for a hard cap.",
        "5. Accept that interval width reflects genuine uncertainty;",
